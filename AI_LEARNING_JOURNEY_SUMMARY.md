...
As of 2026-08-15, the original roadmap is roughly **97% covered**. The strongest completed areas are .NET AI integration, structured outputs, native tool calling, RAG fundamentals, SQL Server native vector search, Qdrant comparison, Kernel Memory comparison, a first deterministic RAG evaluation suite, Angular streaming UX, multi-agent orchestration, MCP read/write tools, OpenTelemetry tracing with Aspire Dashboard visualization, prompt-injection guardrails, first-pass data sanitization, backend performance caching, JWT-backed authorization policies, Angular token propagation, real login with SQL-backed users, admin user management, first-pass rate limiting for expensive AI workloads, durable SQL-backed AI usage logs, daily request and token budgets, estimated AI cost tracking with provider-reported usage fallback, AI usage analytics visibility, integration tests for the new security controls, and a first GitHub Actions CI pipeline.

The latest hardening update added SQL-backed `AppUser` records, password-hashed login, admin-only user management, JWT bearer authentication, capability-based authorization policies, Angular `HttpClient` interceptor support, explicit Bearer-token handling for the raw `fetch()` SSE stream, ASP.NET Core rate limiting for AI/RAG/agent endpoints, SQL-backed AI usage logging with daily request and estimated-token budgets, provider-reported `ChatResponse.Usage` capture when available, estimated cost fields, analytics visibility for AI usage and budget blocks, focused backend integration tests, and a GitHub Actions CI workflow. Smoke tests and integration tests verified anonymous requests are rejected, normal users can read but cannot write, admins can access protected analytics, admins can create users who can then log in, the agent-rate-limit policy returns `429 Too Many Requests` after the configured window is exhausted, the usage-budget filter writes durable audit rows for accepted and budget-exceeded requests, token-budget exhaustion blocks before expensive work runs, provider-reported token counts are aggregated by the usage recorder, and analytics reports today's AI usage by capability and top users.
...
The remaining work is now less about learning basic AI capabilities and more about production maturity: replacing the local password-login system with a real identity provider when needed, extending provider-reported usage capture to streaming/provider-specific SDKs where available, broadening automated tests beyond the first security and deterministic RAG-eval slices, adding deployment/CD, policy-backed PII detection, stream-safe output sanitization, Semantic Kernel Agents / AutoGen .NET comparison, and managed search/vector-store comparisons such as Azure AI Search or Pinecone.
...
- **Deterministic RAG evaluation suite**: `backend.Tests/RagEvaluationTests.cs` now defines a fixed mini knowledge base and repeatable eval cases for retrieval recall@3, mean reciprocal rank, citation/source accuracy, expected answer term coverage, and unsupported-answer hallucination behavior. This gives the RAG pipeline a benchmark that can be rerun whenever retrieval, chunking, reranking, prompts, or vector-store implementations change.
...
- **SQL-backed users and login**: `AppUsers` stores username, email, password hash, role, active status, and daily AI request limit. `POST /api/auth/login` verifies credentials with `IPasswordHasher<AppUser>` and returns a JWT. A seeded local admin account (`admin` / `Admin123!`) is created in Development when the user table is empty.
- **Admin user-management portal**: admins can list, create, and update users from the Angular UI, including role, active status, and per-user daily AI request limits. The backend exposes admin-only `/api/users` endpoints protected by `CanManageUsers`.
- **Development token endpoint**: `POST /api/auth/dev-token` still exists only in Development for tests and local diagnostics, but the UI now uses real login.
...
- **Durable AI usage logs and daily budgets**: a new `AiUsageLogs` SQL table records accepted and budget-exceeded AI/RAG/agent requests by user, role, capability, endpoint, method, status code, timestamps, and duration. The `AiUsageBudgetFilter` checks today's accepted usage before running expensive handlers, using default daily limits of 100 requests for `User` and 500 for `Admin`. Budget exhaustion returns `429 Too Many Requests` before the LLM/embedding/agent work runs.
- **Estimated token and cost budgets**: the usage filter now estimates request-side tokens from endpoint arguments and stores provider name, model name, input/output/total tokens, usage source, and estimated cost. Users now have both daily request limits and daily token limits. Token-budget exhaustion returns `429 Too Many Requests` before the handler runs. Cost defaults to `$0` for local Ollama but the schema is ready for hosted-provider pricing.
- **Provider-reported token usage capture**: non-streaming `ChatResponse` call sites now feed a scoped `AiUsageRecorder`. When `ChatResponse.Usage` exposes input/output/total token counts, `AiUsageLogs` stores those provider-reported counts and marks `TokenUsageSource` as `ProviderReported`; otherwise it falls back to endpoint-filter estimates. Streaming remains estimate-based for now because `ChatResponseUpdate` does not expose usage details in this package version.
- **AI usage analytics**: `/api/analytics/metrics` now includes today's AI request count, budget blocks, unique AI users, estimated token totals, estimated cost totals, usage by capability, and top users. The Angular analytics dashboard renders these as stat cards and compact tables so the budget system is visible, not just enforced silently.
...
| Phase 3: RAG and vector databases | 88% | Strong fundamentals, Qdrant, Kernel Memory comparison, and first deterministic eval suite complete; managed search products and broader real-model evaluation remain. |
...
| Production readiness | 85% | SQL-backed users, password-hashed login, admin user management, AuthZ policies, Angular token propagation, rate limiting, SQL-backed request/token budgets, provider-reported token capture with estimated fallback, estimated AI cost tracking, AI usage analytics, focused security integration tests, GitHub Actions CI, performance caching, and visual trace tooling now exist; biggest remaining gaps are external identity-provider integration, broader provider-specific usage coverage, broader tests, deployment/CD, policy-grade privacy, and operational controls. |
...
- **RAG evaluation**: a first deterministic xUnit evaluation suite now covers retrieval recall@3, mean reciprocal rank, citation/source accuracy, answer term coverage, groundedness, and unsupported-answer behavior. Remaining depth: larger benchmark sets, real endpoint/model eval runs, LLM-as-judge checks, citation-support grading, and trend reports over time.
...
### Cross-cutting
- **Authentication is now app-local, but not externally federated** - SQL-backed users, password hashing, login, JWTs, and admin user management now exist, but there is no external identity provider such as Entra ID, Auth0, or IdentityServer.
- **Token/cost budgeting is partially provider-reported** - non-streaming `ChatResponse.Usage` is captured when available, with endpoint-filter estimates as fallback. Streaming responses and hosted-provider actual billed cost are still not fully provider-reported.
