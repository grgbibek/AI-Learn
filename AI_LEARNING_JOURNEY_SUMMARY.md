...
- **Estimated token and cost budgets**: the usage filter now estimates request-side tokens from endpoint arguments and stores provider name, model name, estimated input/output/total tokens, and estimated cost. Users now have both daily request limits and daily token limits. Token-budget exhaustion returns `429 Too Many Requests` before the handler runs. Cost defaults to `$0` for local Ollama but the schema is ready for hosted-provider pricing.
...
| Production readiness | 84% | SQL-backed users, password-hashed login, admin user management, AuthZ policies, Angular token propagation, rate limiting, SQL-backed request/token budgets, estimated AI cost tracking, AI usage analytics, focused security integration tests, GitHub Actions CI, performance caching, and visual trace tooling now exist; biggest remaining gaps are external identity-provider integration, provider-reported token/cost accounting, broader tests, deployment/CD, policy-grade privacy, and operational controls. |
...
- **Token/cost budgeting is estimated, not provider-reported** - fixed-window rate limits, SQL-backed request budgets, estimated token budgets, and estimated cost fields now exist, but the system does not yet read provider-reported prompt/completion token counts or actual billed cost from hosted AI providers.
